...
@Component({
  selector: 'app-ddcategory',
  templateUrl: './ddcategory.page.html',
  styleUrls: ['./ddcategory.page.scss'],
})
export class DdcategoryPage implements OnInit {
  ...
  deleteCategory() { // In this method use the category id to remove it from firestore
    try{
      this.apicontrollerService.deleteCategory(this.category.id).then(() => {
        this.router.navigateByUrl('/menu/third/dashboard');
        this.showToast('Category removed.');
      }, err => {
        this.showToast('There was a problem removing the category.');
      });
    }
    catch(e){
      alert("Sorry, an application error has occurred."); // Validation in case of an unexpected error
